...
class AHD extends GuideChimp {
  ...
  async renderCarouselBanner(bannerRow: any, identifier?: string) {
    ...
    const makeSlideEl = (s: any, idx: number) => {
    ...
    slides.forEach((s: any, idx: number) => {
      slidesWrap.appendChild(makeSlideEl(s, idx));
    });

    // Seamless forward/backward wrap: append a clone of the first slide after
    // the last, and a clone of the last slide before the first. Wrapping then
    // animates one real step further in the same direction, and once the
    // transition ends we snap (no transition) to the corresponding real slide.
    const canLoop = slides.length > 1;
    if (canLoop) {
      const leadingClone = makeSlideEl(slides[slides.length - 1], slides.length - 1);
      leadingClone.setAttribute('data-clone', 'true');
      const trailingClone = makeSlideEl(slides[0], 0);
      trailingClone.setAttribute('data-clone', 'true');
      slidesWrap.insertBefore(leadingClone, slidesWrap.firstChild);
      slidesWrap.appendChild(trailingClone);
    }
    ...
    // With clones in place, track position 0 is the leading (last-slide) clone,
    // real slides occupy 1..slidesCount, and slidesCount+1 is the trailing
    // (first-slide) clone. Start on the first real slide.
    const cloneOffset = canLoop ? 1 : 0;
    slidesWrap.style.transform = `translateX(${-cloneOffset * 100}%)`;
    ...
    // Jump directly to a real slide index (used for dot/indicator clicks) —
    // always animates via the shortest track delta, no clone pass-through.
    const goTo = (i: number) => {
      if (!state) return;
      const idx = ((i % state.slidesCount) + state.slidesCount) % state.slidesCount;
      state.index = idx;
      state.trackIndex = idx + cloneOffset;
      state.slidesWrap.style.transform = `translateX(${-state.trackIndex * 100}%)`;
      setDots(idx);
    };
    ...
    const carouselBehaviour = bannerRow?.slides?.[0]?.behaviour || bannerRow?.behaviour || {};
    const navigationMode = carouselBehaviour?.navigationMode;
    const shouldAutoAdvance = navigationMode === 'auto' || navigationMode === 'both';
    let navigationDelay = carouselBehaviour?.navigationDelay;
    if (typeof navigationDelay !== 'number' || navigationDelay <= 0) {
      navigationDelay = 3000;
    }
    if (shouldAutoAdvance) {
      state.timer = window.setInterval(() => step(1), navigationDelay);
      carousel.addEventListener('mouseenter', () => { if (state.timer) clearInterval(state.timer); });
      carousel.addEventListener('mouseleave', () => { state.timer = window.setInterval(() => step(1), navigationDelay); });
    }
    ...
    (this as any)._ahd_carousel = { carousel, state, goTo, step, handlers: { onTouchStart, onTouchMove, onTouchEnd } };
  }
  ...
  private async fetchAndCacheAppBannerRow(identifier: string) {
    ...
    LocalStorage.put(
      ...
      nextCache,
      this.options.bannerRefetchIntervalInSec || DEFAULT_BANNER_REFETCH_INTERVAL_IN_SEC
    );
